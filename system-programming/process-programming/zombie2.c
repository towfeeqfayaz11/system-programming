...
int main(){
    ...
    if(id > 0){
        ...
        waitpid(id, NULL, 0); // when parent performs a wait on the child process, then upon termination of child process,
                              // the parent does the cleanup from process table for completed/exited/terminated/zombie child process
        sleep(5);
